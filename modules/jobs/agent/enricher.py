...
from core.config import ANTHROPIC_API_KEY, CANDIDATE_PROFILE, MAX_DAILY_TOKENS
...
def enrich_job_url(url: str) -> str:
    """Fetch a job URL, analyze with Claude, return formatted Telegram message."""
    if get_tokens_used_today() >= MAX_DAILY_TOKENS:
        raise RuntimeError(f"Daily token cap of {MAX_DAILY_TOKENS} reached. No more Claude calls today.")
